...
//import react-redux hooks redux operation
import {useDispatch,useSelector} from 'react-redux'

import endpoint from '../endpoint'


//import redux action from reduz file
import {registerUserAction} from '../redux'

// import axios for hhtp api call to interact with server side
import axios from 'axios'

//functional component register for Register a new user
  ...
  //backend api server url
    ...
    const handleSubmit = async(event) =>{
      event.preventDefault();
        ...
        let result = await axios.post(`${endpoint}/api/auth/register`, user);
        if(!result.data.success){
          return alert(result.data.message);
        }
        let data = result.data.token;

        //if successfull Registration then set the token which got from server to localStorage
        if(data){
